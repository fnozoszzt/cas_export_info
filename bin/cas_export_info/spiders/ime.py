...
class imeSpider(spiders.MySpider):
    ...
    def expert_list_parse(self, response):
        ...
        url = response.url
        expert_list = response.selector.xpath('.//table[@width="96%"]//a')
        for expert in expert_list:
            href = expert.xpath('@href').extract()[0]
            url = urlparse.urljoin(response.url, href)
            yield Request(url, callback=self.expert_info_parse)
        next_page = response.selector.xpath('.//a[@class="STYLE8"]')
        for s in next_page:
    ...
    def analy(self, response):
        ...
        info_list_1 = response.selector.xpath('//table[@class="font03 tablestyle"]//tr')
        for sub_info in info_list_1:
            ...
            if len(l) % 2 == 0:
                for i in range(0, len(l), 2):
                    ...
                    res[key] = value
        info_list_2 = response.selector.xpath('//table[@class="font03"]//table[@class="font03"]')
