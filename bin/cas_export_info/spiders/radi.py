...
class radiSpider(spiders.MySpider):
    ...
    def expert_list_parse(self, response):
        ...
        url = response.url
        expert_list = response.selector.xpath('.//table[@class="bk5"]//a')
    ...
    def analy(self, response):
        ...
        info_list_1 = response.selector.xpath('//td[@class="nrhei"]/table[1]//tr')
        for sub_info in info_list_1:
            ...
            if len(l) % 2 == 0:
                for i in range(0, len(l), 2):
                    ...
                    res[key] = value
        info_list_2 = response.selector.xpath('//td[@class="nrhei"]/table[position()>1]')
